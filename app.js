...
async function fetchCountryData(countryId) {
    ...
    try {
        ...
        alert(`データ読み込みエラー:\n${err.message}\n\ndata/${countryId}.json がindex.htmlと同じ階層にあるか確認してください。`);
        return null;
    }
}

const countryEmergency = {
    thailand: { police: 'tel:191', policeNum: '191', ambulance: 'tel:1669', ambulanceNum: '1669', embassy: 'https://www.th.emb-japan.go.jp/', embassyNum: '☎' },
    korea: { police: 'tel:112', policeNum: '112', ambulance: 'tel:119', ambulanceNum: '119', embassy: 'https://www.kr.emb-japan.go.jp/', embassyNum: '☎' }
};
...
console.log(`🗂️ Found ${countryCards.length} country cards`);
countryCards.forEach(card => {
    card.addEventListener("click", async function() {
        ...
        if (!data) {
            console.error('❌ データ読み込み失敗: ' + country);
            return;
        }
        ...
        if (meta) {
            ...
            if (barFlag) barFlag.textContent = meta.flag;
            if (barName) barName.textContent = meta.name;
        }
    ...
    btn.addEventListener("click", function() {
        ...
        setTimeout(() => {
            ...
            if (targetContent) {
                ...
                if (category === "safety") {
                    const emData = countryEmergency[window.currentCountry];
                    if (emData) updateEmergencyButtons(emData);
                }
            }
        }, 200);
    });
