...
updateDetailCard = function(cardType, data) {
    ...
    setTimeout(() => {
        ...
        console.log('=== 通貨換算デバッグ ===');
        console.log('cardType:', cardType);
        console.log('data.detail:', data.detail);
        console.log('converter存在:', data.detail && data.detail.converter);
        
        // 通貨カードの場合、converter追加
        if (cardType === 'currency' && data.detail && data.detail.converter) {
            console.log('✅ converterデータあり:', data.detail.converter);
            
            setTimeout(() => {
                const detailedInfo = document.getElementById(`detailed-${cardType}`);
                console.log('detailedInfo要素:', detailedInfo);
                
                if (detailedInfo) {
                    const converterHTML = generateCurrencyConverter(data.detail.converter);
                    console.log('converterHTML生成:', converterHTML.substring(0, 100));
                    ...
                    console.log('converterContainer:', converterContainer);
                    
                    if (converterContainer) {
                        setupCurrencyConverter(converterContainer, data.detail.converter);
                        console.log('✅ イベントリスナー設定完了');
                    } else {
                        console.log('❌ converterContainer見つからない');
                    }
                } else {
                    console.log('❌ detailedInfo要素が見つからない');
                }
            }, 100);
        } else {
            console.log('❌ converterスキップ');
            console.log('  - cardType === "currency":', cardType === 'currency');
            console.log('  - data.detail存在:', !!data.detail);
            console.log('  - data.detail.converter存在:', data.detail && !!data.detail.converter);
        }
