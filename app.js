...
if (countryBarChange) {
    countryBarChange.addEventListener('click', () => {
        ...
        countrySelection.classList.remove('hidden');
        countrySelection.style.opacity = '0';
        setTimeout(() => { countrySelection.style.opacity = '1'; }, 50);
    });
}
...
categoryButtons.forEach(btn => {
    btn.addEventListener("click", function() {
        // 初回アニメーション完了後はfirst-loadを外す
        catBtnsContainer.classList.remove('first-load');
        
        const category = this.dataset.category;
        
        // フェードアウトカテゴリー
        catBtnsContainer.style.opacity = '0';
        catBtnsContainer.style.transition = 'opacity 0.2s ease';
        
        setTimeout(() => {
            catBtnsContainer.style.display = "none";
            catBtnsContainer.style.opacity = '';
            catBtnsContainer.style.transition = '';
...
backButtons.forEach(btn => {
    btn.addEventListener("click", () => {
        ...
        setTimeout(() => {
            ...
            catBtns.style.opacity = '0';
            catBtns.style.transition = 'opacity 0.25s ease';
            // scrollを上に
            window.scrollTo({ top: 0, behavior: 'auto' });
            requestAnimationFrame(() => {
                catBtns.style.opacity = '1';
            });
        }, 200);
    });
});
...
function showBasicInfo(cardType) {
    ...
    if (basicEl && detailedEl) {
        ...
        detailedEl.style.transition = 'opacity 0.2s ease';
        
        setTimeout(() => {
            detailedEl.classList.remove("fade-in");
            detailedEl.classList.add("hidden");
            detailedEl.style.opacity = '';
            detailedEl.style.transition = '';
