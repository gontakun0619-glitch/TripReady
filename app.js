...
function showDebug(message, isError = false) {
    ...
    // document.bodyが存在する場合のみDIVを作成
    if (!document.body) {
        return; // bodyがまだない場合は後で処理
    }
    ...
    debugDiv.innerHTML = debugMessages.map(msg => 
        `<div style="margin-bottom: 5px; color: ${msg.startsWith('❌') ? '#ff0000' : msg.startsWith('✅') ? '#00ff00' : '#ffffff'}">${msg}</div>`
    ).join('');
    debugDiv.scrollTop = debugDiv.scrollHeight;
}

// DOMが読み込まれたらデバッグDIVを作成
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', function() {
        showDebug('🔄 DOMContentLoaded - デバッグDIVを作成');
        // すでに保存されているメッセージを表示
        debugMessages.forEach(msg => showDebug(msg));
    });
}
...
window.addEventListener('error', function(e) {
    if (typeof showDebug === 'function') {
        showDebug('❌ JavaScriptエラー: ' + e.message, true);
        showDebug('  ファイル: ' + e.filename, true);
        showDebug('  行: ' + e.lineno, true);
    }
    return false;
});
...
window.addEventListener('unhandledrejection', function(e) {
    if (typeof showDebug === 'function') {
        showDebug('❌ Promise エラー: ' + e.reason, true);
    }
});

showDebug('【STEP 0】app.js読み込み開始');
showDebug('========================================');
...
// 国データ
showDebug('【STEP 1】countryData定義開始');
showDebug('  - thailandData exists? ' + (typeof thailandData !== 'undefined'));
showDebug('  - koreaData exists? ' + (typeof koreaData !== 'undefined'));
if (typeof thailandData !== 'undefined') {
    showDebug('  - thailandData.name: ' + thailandData.name);
}
if (typeof koreaData !== 'undefined') {
    showDebug('  - koreaData.name: ' + koreaData.name);
}
...
// 以下、既存のUIロジック
showDebug('【STEP 2】DOM要素取得開始');
const openingScreen = document.getElementById("opening-screen");
showDebug('  - openingScreen: ' + (openingScreen ? '✅ 取得成功' : '❌ null'));
if (!openingScreen) {
    showDebug('❌ エラー：openingScreenが見つかりません！', true);
}
const countrySelection = document.getElementById("country-selection");
showDebug('  - countrySelection: ' + (countrySelection ? '✅ 取得成功' : '❌ null'));
if (!countrySelection) {
    showDebug('❌ エラー：countrySelectionが見つかりません！', true);
}
...
// オープニングアニメーション
showDebug('【STEP 3】オープニングアニメーション設定');
showDebug('  - 1.5秒後に実行されます...');
setTimeout(() => {
    showDebug('【STEP 4】オープニングアニメーション実行！');
    showDebug('  - openingScreen.classList.add("slide-up") を実行');
    openingScreen.classList.add("slide-up");
    showDebug('  - ✅ slide-upクラスを追加しました');
    showDebug('  - openingScreen.classList: ' + Array.from(openingScreen.classList).join(', '));
    setTimeout(() => {
        openingScreen.style.display = "none";
        showDebug('【STEP 5】国選択画面を表示');
        countrySelection.classList.remove("hidden");
        showDebug('  - ✅ hiddenクラスを削除');
        countrySelection.classList.add("show");
        showDebug('  - ✅ showクラスを追加');
        showDebug('========================================');
        showDebug('【完了】オープニングアニメーション完了！');
        showDebug('========================================');
    }, 600);
}, 1500);
